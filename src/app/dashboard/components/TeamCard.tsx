...
export default function TeamCard({ team, onEdit, onDelete }: TeamCardProps) {
  ...
  const getImageUrl = () => {
    if (team.imageUrls && team.imageUrls.length > 0) {
      try {
        new URL(team.imageUrls[0]);
        return team.imageUrls[0];
      } catch {
        ...
        <div className={styles.contactInfo}>
          {team.contact?.phone && (
            <div className={styles.contactItem}>
              ...
              <span className={styles.contactValue}>{team.contact.phone}</span>
            </div>
          )}
          ...
          {team.contact?.email && (
            <div className={styles.contactItem}>
              ...
              <span className={styles.contactValue}>{team.contact.email}</span>
            </div>
          )}
        </div>
        
        {(team.socialMedia?.linkedin || team.socialMedia?.twitter || team.socialMedia?.facebook || team.socialMedia?.instagram) && (
          <div className={styles.socialLinks}>
            {team.socialMedia?.linkedin && (
              <a
                href={team.socialMedia.linkedin}
                target="_blank"
